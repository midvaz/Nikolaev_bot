...
from messege import messege_help
...
async def get_chat_id(message: types.Message):
    await bot.send_message(
        ...
        text=f"Id этого чата: {message.chat.id}"
    )
...
async def add_new_wallet(*args, **kwargs):
    ...
    if data:
        ...
        if data[0].upper() in chains:
            async with session_maker() as session:
                wallets_query = select(Wallets.wallet).where(
                    Wallets.user == user.user_id,
            ...
            if data[1] not in current_wallets:
                ...
                await bot.send_message(
                    chat_id, 
                    f"Кошелек `{data[1]}` в сети {data[0]} добавлен с именем `{w_name}`",
                    parse_mode="MarkDown"
                )
            else:
                async with session_maker() as session:
                    ...
                    await bot.send_message(
                        chat_id, 
                        f"`{data[1]}` уже существует с именем `{name}`",
                        parse_mode="MarkDown"
                    )
        else:
            await bot.send_message(
                chat_id, 
                f"{data[0]} - сеть указана не верно"
            )
...
async def remove_wallet(*args, **kwargs):
    ...
    if data:
        ...
        await bot.send_message(
            chat_id, 
            "Пожалуйста, укажите адрес в формате /remove_wallet <сеть> <адрес>!"
        )
    ...
    if data:
        ...
        async with session_maker() as session:
            ...
            if user is not None:
                await bot.send_message(
                    chat_id, 
                    f"Пользователь {user_id} уже существует с именем '{user.user_name}'"
                )
            else:
                query = insert(Users).values(
                    user_id=user_id, chat_id=user_id, 
                    is_admin=False, user_name=" ".join(data[1:])
                )
                await session.execute(query)
                ...
                await bot.send_message(
                    chat_id, 
                    f'Пользователь {user_id} добавлен с именем "{" ".join(data[1:])}"'
                )
    else:
        await bot.send_message(
            chat_id,
    ...
    if data:
        ...
        async with session_maker() as session:
            ...
            if user is not None:
                ...
                query = insert(Users).values(
                    user_id=user_id, chat_id=user_id, 
                    is_admin=True, user_name=" ".join(data[1:])
                )
                await session.execute(query)
                ...
                await bot.send_message(
                    chat_id, 
                    f'Админ {user_id} добавлен с именем "{" ".join(data[1:])}"'
                )
    else:
        await bot.send_message(
            chat_id,
    ...
    if data:
        ...
        async with session_maker() as session:
            ...
            if user_id not in res:
                ...
                await bot.send_message(
                    chat_id, 
                    f'Канал {channel_id} с группой {user_id} был удален. '
                    f'Также удалены все связанные с ним кошельки'
                )
    else:
        await bot.send_message(
...
async def help_message(*args, **kwargs):
    ...
    if not data:
        ...
        if (not(user.is_admin) and (
            data[0]== 'add_user' or data[0]== 'remove_user' 
            or data[0]== 'add_admin' or data[0]== 'remove_admin' 
            or data[0]== 'add_group' or data[0]== 'remove_group'  or data[0]== 'get_id'
            or data[0]== 'get_users' or data[0]== 'get_wallets_all') 
        ):
            return
        
        else:
            t = messege_help[f'/help {data[0]}']
...
async def continue_tracking_after_restart(*args, **kwargs):
    ...
    if continueted:
        await bot.send_message(
            ...
            text="Уже использованно"
        )
        return
    
    continueted = True
    await bot.send_message(chat_id=ID_G,text="Restarted")
    tasks = []
